...
from detectors.Yolov5_StrongSORT_OSNet.yolov5.utils.segment.general import masks2segments, process_mask, process_mask_native
...
class Hir_Yolo:
    ...
    @torch.no_grad()
    def interface_people(self, dataset):
        """
        扫描框展示
        """
        hide_labels = False
    ...
    def load_video(self, media_source):
        data_array = []
        stride, pt = self.model.stride, self.model.pt
        dataset = LoadImages(media_source, img_size=self.imgsz, stride=stride, auto=pt)
        for i in dataset:
            data_array.append(i)
        return data_array
